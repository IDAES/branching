...
auto sum_positive_negative = [](auto const& row_cols, auto const& row_cols_vals) {
	auto positive_sum = value_type{0.};
	auto negative_sum = value_type{0.};
...
template <typename Tensor> void set_number_constraints(Tensor&& out, SCIP_COL* const col) noexcept {
	out[idx(Features::n_rows)] = static_cast<value_type>(SCIPcolGetNLPNonz(col));
}
...
void set_static_stats_for_constraint_degree(Tensor&& out, nonstd::span<SCIP_ROW*> const rows) noexcept {
	...
	auto row_get_nnonfixed_cols = [](auto const row) {
		int ctr = 0;
		auto const row_cols = scip::get_cols(row);
		for (auto* const col: row_cols) {
			if(SCIPcolGetLPPos(col) >= 0) {
				if (SCIPcolGetLb(col) + 0.5 > SCIPcolGetUb(col))
					continue;
				ctr ++;
	...
	auto const stats = utility::compute_stats(rows | ranges::views::transform(row_get_nnonfixed_cols));
...
template <typename Tensor> void set_static_features(Tensor&& out, SCIP_COL* const col) {
	...
	auto const n_col_lp_rows = SCIPcolGetNLPNonz(col);

	auto const filtered_rows = views::take(rows, n_col_lp_rows);
	auto const filtered_coefficients = views::take(coefficients, n_col_lp_rows);

	set_objective_function_coefficient(out, col);
	set_number_constraints(out, col);
	set_static_stats_for_constraint_degree(out, filtered_rows);
	set_stats_for_constraint_positive_coefficients(out, filtered_coefficients);
	set_stats_for_constraint_negative_coefficients(out, filtered_coefficients);
}
...
void set_dynamic_stats_for_constraint_degree(Tensor&& out, nonstd::span<SCIP_ROW*> const rows) noexcept {
	...
	auto const stats = utility::compute_stats(rows | views::transform(row_get_nnonfixed_cols));
...
void set_min_max_for_ratios_constraint_coeffs_rhs(
	...
	for (auto const [row, coef] : views::zip(rows, coefficients)) {
		...
		for (auto const [val, col] : views::zip(row_values, row_cols))  { 
			if(SCIPcolGetLPPos(col) >= 0) {
				if (SCIPcolGetLb(col) + 0.5 > SCIPcolGetUb(col)) //fixed column!
					temp_sum += val * SCIPcolGetLb(col); //value..
			}
		}

		if (auto const rhs = SCIProwGetRhs(row); !SCIPisInfinity(scip, std::abs(rhs))) {
			//printf("My Id: %d Rhs: %f Red rhs: %f\n", SCIProwGetLPPos(row), rhs, rhs - temp_sum);
			rhs_ratio_updates(coef, rhs - SCIProwGetConstant(row) - temp_sum);
		}
		if (auto const lhs = SCIProwGetLhs(row); !SCIPisInfinity(scip, std::abs(lhs))) {
			//printf("My Id: %d Lhs: %f Red lhs: %f\n", SCIProwGetLPPos(row),lhs, lhs - temp_sum);
			// lhs constraints are multiply by -1 to be considered as rhs constraints.
			rhs_ratio_updates(-coef, -1 * (lhs - SCIProwGetConstant(row)) + temp_sum);
		}
...
void set_min_max_for_one_to_all_coefficient_ratios(
	...
	for (auto const [row, coef] : views::zip(rows, coefficients)) {
		...
		auto const n_row_lp_cols = static_cast<value_type>(SCIProwGetNLPNonz(row));

		auto const filtered_row_values = views::take(row_values, n_row_lp_cols);
		auto const filtered_row_cols = views::take(row_cols, n_row_lp_cols);


		auto const [positive_coeficients_sum, negative_coeficients_sum] = sum_positive_negative(filtered_row_cols, filtered_row_values);
		if (coef > 0) {
 ...
 */
auto stats_for_active_constraint_coefficients_weights(scip::Model& model, bool pseudo) { //Selin added
	auto* const scip = model.get_scip_ptr();
	auto const lp_rows = model.lp_rows();
	auto const candidates = pseudo ? model.pseudo_branch_cands() : model.lp_branch_cands(); // SELIN added
	auto const branch_candidates = candidates | ranges::to<std::set>(); //SELIN changed, normally model.pseudo_branch_cands()
	...
	auto sum_abs = [](auto const& row_cols, auto const& row_cols_vals) {
	...
	for (auto* const row : lp_rows) { //OK, only lp rows
		if (row_is_active(scip, row)) {

			auto const red_norm = row_get_reduced_norm(row);
			...
			*(weights_iter++) = 1.;
			*(weights_iter++) = safe_inv(safe_div(sum_abs(scip::get_cols(row), filtered_row_values), red_norm));
			*(weights_iter++) = safe_inv(safe_div(sum_abs_if_candidate(scip::get_cols(row), filtered_row_values), red_norm));
	...
	if (n_active_rows > 0) {
		...
		for (auto const [row, coef] : views::zip(rows, coefficients)) {
			...
			if (row_is_active(scip, row)) {
				for (std::size_t weight_idx = 0; weight_idx < weights_stats.size(); ++weight_idx) {
					auto const weight = lp_rows_weights(row_lp_idx, weight_idx);
					assert(!std::isnan(weight));  // If NaN likely hit a masked value?
					auto const weighted_abs_coef = weight * std::abs(coef);

					auto& stats = weights_stats[weight_idx];
					stats.stddev += square(weighted_abs_coef - stats.mean); // SELIN changed 
				}
			}
		}
...
void set_dynamic_features(
	...
	set_dynamic_stats_for_constraint_degree(out, filtered_rows);
	set_min_max_for_ratios_constraint_coeffs_rhs(out, scip, filtered_rows, filtered_coefficients);
	set_min_max_for_one_to_all_coefficient_ratios(out, filtered_rows, filtered_coefficients);
	set_stats_for_active_constraint_coefficients(out, scip, filtered_rows, filtered_coefficients, lp_rows_weights);
}
...
auto extract_all_features(scip::Model& model, bool pseudo, xt::xtensor<value_type, 2> const& static_features) {
	...
	auto* const scip = model.get_scip_ptr();
	auto const lp_rows_weights = stats_for_active_constraint_coefficients_weights(model, pseudo);
