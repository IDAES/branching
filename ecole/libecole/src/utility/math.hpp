...
//using range_value_type_t = decltype(*std::declval<R>().begin());
}  // namespace internal
...
auto compute_stats(Range range) noexcept -> StatsFeatures<T> {
	...
	auto const mean = safe_div(static_cast<T>(sum), static_cast<T>(count)); // auto const mean
	auto stddev = T{0.};
	auto min = std::numeric_limits<U>::max(); //min
	auto max = std::numeric_limits<U>::lowest(); //std::numeric_limits<U>::lowest() ??
